...
def plot_option_strategy(strategy_function, settlement_prices: iter, kwargs):
    ...
    option_payoffs = [
        strategy_function(settlement, **at_expiry_kwargs) - initial_premium
        for settlement in settlement_prices
    ]
...
def plot_deltas(current_strike, stock_prices, deltas: float, option_type: str):
    ...
    plt.axvline(
        current_strike, color="black", linestyle="dashed", linewidth=2, label="Strike"
    )
    plt.legend()
